...
from ibgnn_spatio_temporal.lib.nn.layers import VariationalNodeEmbedding, ClusterizedNodeEmbedding
from ibgnn_spatio_temporal.lib.nn.losses import kl_divergence_normal
from ibgnn_spatio_temporal.lib.nn.models import STGNN
...
class EmbeddingPredictor(Predictor):
    ...
    def __init__(self,
        ...
        self.add_params_epoch = add_params_epoch
    ...
    def add_ibg_params(self):
        self.model.mp_layers[0].allow_ibg_approx_training()
        ibg_params = list(self.model.mp_layers[0].get_ibg_approx_parameters())
        optimizers = ensure_list(self.optimizers())
        
        for optimizer in optimizers:
            for param_group in optimizer.param_groups:
                for param in ibg_params:
                    for i, p in enumerate(param_group['params']):
                        if torch.equal(p, param):
                            param_group['params'].pop(i)

        for optimizer in optimizers:
            lr = optimizer.optimizer.param_groups[0]['lr']
            optimizer.add_param_group({'params': ibg_params, 'lr': lr / 100})
            # optimizer.add_param_group({'params': ibg_params})
    ...
    def on_train_epoch_start(self) -> None:
        ...
        if self.add_params_epoch > 0:
            if (self.current_epoch == self.add_params_epoch):
                self.add_ibg_params()
