# Code adapted from "Edge Directionality Improves Learning on Heterophilic Graphs"
# https://github.com/emalgorithm/directed-graph-neural-network/blob/main/src/utils/third_party.py#L54
...
def load_snap_patents_mat(n_classes=5, root="dataset/", pre_transform=None):
    ...
    root = "folds/"
    name = "snap-patents"
    if not os.path.exists(f"{root}{name}-splits.npy"):
    ...
    # Get splits
    splits_lst = np.load(f"{root}{name}-splits.npy", allow_pickle=True)
