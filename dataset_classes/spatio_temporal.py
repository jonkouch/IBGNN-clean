...
def load_dataset_by_name(dataset_name: str):
    ...
    if dataset_name == 'la':
        ...
        dataset = AirQuality(root=dataset_path, impute_nans=True)
    else:
        raise ValueError(f'DataSetType {dataset_name} not supported in ibg_approx_load')
    return dataset


def ibg_approx_load(dataset_name: str, pre_transform: Optional[BaseTransform] = None, batch_size: int = 32, window: int = 24,
                horizon: int = 3, stride: int = 1, threshold: int = 0.1, symmetric: bool = True) -> List[Data]:
...
def ibgnn_load(dataset_name: str, batch_size: int = 32, window: int = 24, horizon: int = 3,
