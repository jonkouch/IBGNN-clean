...
from ibgnn.set_layers import DeepSets
...
class basic(Module):
    def __init__(self, in_dim: int, hidden_dim: int, out_dim: int, num_layers: int, dropout: float, skip_com: bool,
...
class deepsets(Module):
    def __init__(self, in_dim: int, hidden_dim: int, out_dim: int, num_layers: int, dropout: float, skip_com: bool,
                 act_type: ActivationType, pool: str):
        super().__init__()
        self.add_encoder = in_dim != hidden_dim
        list_of_modules = []
        if self.add_encoder:
            list_of_modules += [Linear(in_features=in_dim, out_features=hidden_dim)]
        list_of_modules += [DeepSets(in_dim=hidden_dim, out_dim=hidden_dim, pool=pool)
                            for _ in range(num_layers)]
        list_of_modules += [Linear(in_features=hidden_dim, out_features=out_dim)]
        self.module_list = ModuleList(list_of_modules)
        self.dropout = Dropout(dropout)
